...
(defun tws-refresh-buffer ()
  ...
  (with-output-to-temp-buffer *tws-buffer-name*
    (with-current-buffer *tws-buffer-name*
      ...
      (local-set-key (kbd "P") 'tws-pop-last-shift-on-line)
      (local-set-key (kbd "/") 'tws-filter-goals)
      (switch-to-buffer *tws-buffer-name*)))
  (goto-char *tws-last-known-point*))

(defun tws-pop-last-shift-on-line ()
  (interactive)
  (let ((entry (tws-entry-on-line)))
    (when (and  entry (y-or-n-p "Really drop last shift?"))
      (pop (tws-entry-time entry)))))
...
(defun tws-help-buffer ()
  ...
  (with-output-to-temp-buffer "Time Well Spent: Help"
    (with-current-buffer "Time Well Spent: Help"
      ...
      (princ "COMMANDS")
      (terpri)
      (princ "+        : Create a new goal")      (terpri)
      (princ "<return> : Toggle tracking on a goal.")      (terpri)
      (princ "S        : Stop tracking")      (terpri)
      (princ "A        : Show all entries - i.e. remove filtering")      (terpri)
      (princ "<SPC>    : Toggle filter by category of highlghted entry")      (terpri)
      (princ "C        : Toggle filter by completed goals")      (terpri)
      (princ "F        : Toggle filter by future goals")      (terpri)
      (princ "D        : Default filter")      (terpri)
      (princ "m        : Toggle an entry as being marked as completed")      (terpri)
      (princ "f        : Mark an entry as a future goal")      (terpri)
      (princ "d        : Mark an entry as a goal you're definitely doing")      (terpri)
      (princ "w        : Mark an entry as being in wating, enter reason at prompt")      (terpri)
      (princ "l        : Add a log message for the highlighted entry")      (terpri)
      (princ "R        : Show a category report")      (terpri)
      (princ "r        : Rename goal for the highlighted entry")      (terpri)
      (princ "M        : Move the highlighted entry to a different category")      (terpri)
      (princ "v        : View the detail of the highlighted entry")      (terpri)
      (princ "a        : Add time to highlighted entry")      (terpri)
      (princ "q        : close the Time Well Spent buffer (will continue tracking)")      (terpri)
      (princ "a        : manually add time to a goal") (terpri)
      (princ "X        : remove this goal") (terpri)
      (princ "P        : remove the last logged entry from thsi goal") (terpri)
